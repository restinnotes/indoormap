...
class HybridPdrEngine(private val context: Context) : SensorEventListener {
    ...
    // Callback with: normal, heading, rawQ, smoothQ, state, motionVariance, headingDeg
    fun onSwingPlaneDetected(normal: FloatArray, heading: FloatArray, rawQ: Float, smoothQ: Float, state: SwingState, motionVar: Float, headingDeg: Float) {
        this.swingState = state
        // Format: SWING_PLANE,nx,ny,nz,hx,hy,hz,rawQ,smoothQ,state,motionVar,headingDeg
        listener?.onDebugMessage("SWING_PLANE,${normal[0]},${normal[1]},${normal[2]},${heading[0]},${heading[1]},${heading[2]},$rawQ,$smoothQ,$state,$motionVar,$headingDeg")
    }
    ...
    fun solveEigen3x3(xx: Float, xy: Float, xz: Float, yy: Float, yz: Float, zz: Float): EigenResult {
        ...
        for (iter in 0 until 5) {
             if (Math.abs(m01) > 1e-6) {
                 ...
                 val d0_ = c*c*d0 - 2*c*s*m01 + s*s*d1; val d1_ = s*s*d0 + 2*c*s*m01 + c*c*d1
                 d0=d0_; d1=d1_; m01=0f
                 val m02_ = c*m02 - s*m12; val m12_ = s*m02 + c*m12; m02=m02_; m12=m12_
             ...
             if (Math.abs(m02) > 1e-6) {
                 ...
                 val c = cos(theta).toFloat(); val s = sin(theta).toFloat()
                 val d0_ = c*c*d0 - 2*c*s*m02 + s*s*d2; val d2_ = s*s*d0 + 2*c*s*m02 + c*c*d2
                 d0=d0_; d2=d2_; m02=0f
                 val m01_ = c*m01 - s*m12; val m12_ = s*m01 + c*m12; m01=m01_; m12=m12_
             ...
             if (Math.abs(m12) > 1e-6) {
                 ...
                 val c = cos(theta).toFloat(); val s = sin(theta).toFloat()
                 val d1_ = c*c*d1 - 2*c*s*m12 + s*s*d2; val d2_ = s*s*d1 + 2*c*s*m12 + c*c*d2
                 d1=d1_; d2=d2_; m12=0f
                 val m01_ = c*m01 - s*m02; val m02_ = s*m01 + c*m02; m01=m01_; m02=m02_
        ...
        for(i in 0..1) { for(j in 0 until 2-i) { if(valArr[j] > valArr[j+1]) { val t = valArr[j]; valArr[j]=valArr[j+1]; valArr[j+1]=t; val tv = vecArr[j]; vecArr[j]=vecArr[j+1]; vecArr[j+1]=tv } } }
    ...
    class SwingPlaneDetector(val engine: HybridPdrEngine) {
        ...
        fun addSample(ax: Float, ay: Float, az: Float) {
            ...
            axBuffer[ptr] = ax; ayBuffer[ptr] = ay; azBuffer[ptr] = az
        ...
        private fun processWindow() {
            ...
            for (i in 0 until WINDOW_SIZE) { sumX += axBuffer[i]; sumY += ayBuffer[i]; sumZ += azBuffer[i] }
            val meanX = sumX / WINDOW_SIZE; val meanY = sumY / WINDOW_SIZE; val meanZ = sumZ / WINDOW_SIZE
            val gravity = floatArrayOf(meanX, meanY, meanZ)

            var xx=0f; var xy=0f; var xz=0f; var yy=0f; var yz=0f; var zz=0f
            for (i in 0 until WINDOW_SIZE) {
                val dx = axBuffer[i] - meanX; val dy = ayBuffer[i] - meanY; val dz = azBuffer[i] - meanZ
